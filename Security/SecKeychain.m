...
/*
  ...
  There is no index, so to find an item you have to scan down the list of
  GUID's.
 */
...
+ (SecKeychain *) defaultUserKeychain {
    NSArray *path = [NSArray arrayWithObjects: @"Software", @"Cocotron",
                                               @"Keychains", @"login", nil];
...
- (NSString *) createGUID {
    ...
    NSString *result = [NSString stringWithCharacters: uString
                                               length: unicodeLength(uString)];
...
static void *encryptData(void *bytes, unsigned length, unsigned *resultLength) {
    ...
    *resultLength = length;
    ...
    if (!CryptProtectData(&plaintext, L"Keychain encrypted data", NULL, NULL,
                          NULL, 0, &crypted)) {
        *resultLength = 0;
        return NULL;
    }
...
static void *decryptData(void *bytes, unsigned length, unsigned *resultLength) {
    ...
    *resultLength = length;
    ...
    if (!CryptUnprotectData(&crypted, &description, NULL, NULL, NULL, 0,
                            &plaintext)) {
                                ...
                                itemClass: (SecItemClass) itemClass
{
    ...
    for (i = 0;; i++) {
        ...
        error = RegEnumValueW(handle, i, nameBuffer, &nameLength, NULL, &type,
                              NULL, &dataLength);
        ...
        nameLength = 256;
        if (RegEnumValueW(handle, i, nameBuffer, &nameLength, NULL, &type, data,
             ...
             attributeList: (const SecKeychainAttributeList *) attributeList
{
    ...
    while (result == nil) {
        ...
        if (RegEnumKeyExW([cursor handle], [cursor index], nameBuffer,
                          &nameBufferLength, NULL, NULL, NULL,
                          NULL) != ERROR_SUCCESS)
            return nil;

        [cursor incrementIndex];
        NSString *guid = [NSString stringWithCharacters: nameBuffer
                                                 length: nameBufferLength];
        NSMutableArray *path = [NSMutableArray array];
        ...
        SecKeychainItemRef item = [self createItemAtHandle: checkHandle
                                                 itemClass: [cursor itemClass]];
