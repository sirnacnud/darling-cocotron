...
- createCFProperty: name {
    ...
    if ([name isEqual: CFSTR(kIOPlatformSerialNumberKey)]) {
        ...
        if (data == nil)
            ...
            // **** WARNING: DO NOT CHANGE THIS. Application software may depend
            // on the serial number for cryptography. The serial number is the
            // primary MAC address with each nibble inverted in hex format
            ...
            return [[NSString alloc] initWithCharacters: buffer
                                                 length: length * 2];
        }
    }
