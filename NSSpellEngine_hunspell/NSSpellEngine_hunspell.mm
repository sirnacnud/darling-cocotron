...
- (NSSpellEngine_hunspellDictionary *) _dictionaryForLanguage:
    ...
    if (dict == nil) {
        ...
        // no canadian or belgium dictionary is available
...
- (NSArray *) checkString: (NSString *) stringToCheck
    ...
    // The letterCharacterSet seems to be missing some marks. Most glaringly the
    // apostrophe So we'll create another character set where we can add marks
    // that cause spellcheck to fail. There seems to be no easy way to update
