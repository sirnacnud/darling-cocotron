...
//{
//    IMP imp = class_getMethodImplementation([self class],
//    @selector(forwardTest)); //(id)((int)[self forwardTest]); return imp(self,
//    @selector(forwardTest));
//}
...
+ (NSString *) doSomethingWith: (NSString * (^)(NSString *) ) block {
    return [NSString stringWithFormat: @"The block output: %@",
                                       block(@"the block argument")];
}
...
int main(int argc, char **argv) {
    ...
    // NSSelectorFromString(@"nix")]); NSLog(@"-- %lld --", [[Hello world]
    ...
    NSLog(@"The call with block output: %@",
          [Blocker doSomethingWith: ^(NSString *arg) {
            return [NSString stringWithFormat: @"The block got: %@", arg];
          }]);
#else
    NSLog(@"You did not compile with clang … be assured that you are missing "
          @"something.");
#endif
