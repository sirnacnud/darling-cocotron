...
// Modified for O2Context_AntiGran to match default Cocotron GDI fonts default
// value and to get glyph by glyph index, and not ASCII code
    ...
    while (cur_glyph < end_glyph) {
        ...
        while (cur_poly < end_poly) {
            ...
            if (pc->wType == TT_PRIM_QSPLINE) {
                int u;
                for (u = 0; u < pc->cpfx - 1;
                     u++) // Walk through points in spline
                {
                    POINTFX pnt_b =
                        pc->apfx[u]; // B is always the current point
                    ...
                    if (u < pc->cpfx - 2) // If not on last spline, compute C
                    {
                        // midpoint (x,y)
                        *(int *) &pnt_c.x =
                            (*(int *) &pnt_b.x + *(int *) &pnt_c.x) / 2;
                        *(int *) &pnt_c.y =
                            (*(int *) &pnt_b.y + *(int *) &pnt_c.y) / 2;
                    }
...
//------------------------------------------------------------------------
font_engine_win32_tt_base::font_engine_win32_tt_base(bool flag32, HDC dc,
                                                     unsigned max_fonts)
    : m_flag32(flag32), m_dc(dc),
      m_old_font(m_dc ? (HFONT)::GetCurrentObject(m_dc, OBJ_FONT) : 0),
      m_fonts(new HFONT[max_fonts]), m_num_fonts(0), m_max_fonts(max_fonts),
      m_font_names(new char *[max_fonts]), m_cur_font(0),

      m_change_stamp(0), m_typeface(new char[256 - 16]),
      m_typeface_len(256 - 16 - 1), m_signature(new char[256 + 256 - 16]),
      m_height(0), m_width(0), m_weight(FW_REGULAR), m_italic(false),
      m_char_set(DEFAULT_CHARSET),
      m_pitch_and_family(DEFAULT_PITCH | FF_DONTCARE), m_hinting(true),
      m_flip_y(false), m_font_created(false), m_resolution(0),
      m_glyph_rendering(glyph_ren_native_gray8), m_glyph_index(0),
      m_data_size(0), m_data_type(glyph_data_invalid), m_bounds(1, 1, 0, 0),
      m_advance_x(0.0), m_advance_y(0.0), m_gbuf(new char[buf_size]),
      m_kerning_pairs(0), m_num_kerning_pairs(0), m_max_kerning_pairs(0),
...
int font_engine_win32_tt_base::find_font(const char *name) const {
    unsigned i;
    for (i = 0; i < m_num_fonts; ++i) {
        if (strcmp(name, m_font_names[i]) == 0)
            return i;
    }
    return -1;
}
        ...
        if (idx >= 0) {
            ...
            if (m_cur_font) {
                if (m_num_fonts >= m_max_fonts) {
                    ...
                    memmove(m_fonts, m_fonts + 1,
                            (m_max_fonts - 1) * sizeof(HFONT));
                    memmove(m_font_names, m_font_names + 1,
...
void font_engine_win32_tt_base::update_signature() {
    ...
    if (m_dc && m_cur_font) {
        ...
        sprintf(m_signature, "%s,%u,%d,%d:%dx%d,%d,%d,%d,%d,%d,%08X",
                m_typeface, m_char_set, int(m_glyph_rendering), m_resolution,
                m_height, m_width, m_weight, int(m_italic), int(m_hinting),
                int(m_flip_y), int(m_pitch_and_family), gamma_hash);
...
#define GGO_UNHINTED 0x0100
        ...
        GLYPHMETRICS gm;
        int total_size = GetGlyphOutlineX(m_dc, glyph_code, format, &gm,
                                          buf_size, (void *) m_gbuf, &m_matrix);
        ...
        if (total_size < 0) {
            ...
            int total_size = GetGlyphOutlineX(
                m_dc, glyph_code, GGO_METRICS | GGO_GLYPH_INDEX, &gm, buf_size,
                (void *) m_gbuf, &m_matrix);
        ...
        case glyph_ren_native_mono:
            decompose_win32_glyph_bitmap_mono(
                m_gbuf, gm.gmBlackBoxX, gm.gmBlackBoxY, gm.gmptGlyphOrigin.x,
                m_flip_y ? -gm.gmptGlyphOrigin.y : gm.gmptGlyphOrigin.y,
        ...
        case glyph_ren_outline:
            ...
            if (m_flag32) {
                m_path32.remove_all();
                if (decompose_win32_glyph_outline(m_gbuf, total_size, m_flip_y,
                                                  m_affine, m_path32)) {
        ...
        case glyph_ren_agg_mono:
            ...
            if (m_flag32) {
                m_path32.remove_all();
                decompose_win32_glyph_outline(m_gbuf, total_size, m_flip_y,
                                              m_affine, m_path32);
