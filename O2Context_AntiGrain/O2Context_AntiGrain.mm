...
// Like NSLog, but works in C++ code - format is a C strings (""), not a Obj-C
// string (@"")
#ifdef DEBUG
static void O2Log(const char *format, ...) {
    va_list ap;
    va_start(ap,
             format); // Requires the last fixed parameter (to get the address)
...
// Multiply the alpha spans with a ratio - used as a transformer to render alpha
// for non-plain color
typedef agg::scanline_u8_am<MaskType> scanline_mask_type;

// render_scanlines_aa_solid with some added translation parameter - we've just
// added the "+dx, +dy" part
            ...
            for (;;) {
                ...
                if (span->len > 0) {
                    ren.blend_solid_hspan(x + dx, y + dy, (unsigned) span->len,
                                          ren_color, span->covers);
                } else {
                    ren.blend_hline(x + dx, y + dy,
                                    (unsigned) (x - span->len - 1), ren_color,
                                    *(span->covers));
                }
                if (--num_spans == 0)
                    break;
...
// render_scanlines_aa with some added translation parameter - we've just added
// the "+dx, +dy" part
...
// Replace spans with a fixed color, keeping the original opacity - used as a
// transformer to render shadow with no blur, for non-plain color
...
class context_renderer {
    ...
    class context_renderer_helper : public context_renderer_helper_base {
        typedef o2agg::pixfmt_custom_blend_rgba<blender_type,
      ...
      public:
        ...
        void setUpShadow() {
            if (ren_shadow == NULL) {
                ...
                renderingBufferShadow = new agg::rendering_buffer(
                    pixelShadowBytes, width, height, bytesPerRow);

                pixelFormatShadow =
                    new pixfmt_shadow_type(*renderingBufferShadow);
                ren_shadow = new renderer_shadow(*pixelFormatShadow);
            }
            ren_shadow->reset_clipping(1);
            ren_shadow->clip_box(ren_base->xmin(), ren_base->ymin(),
                                 ren_base->xmax(), ren_base->ymax());
        }
        ...
        void blur(S &sl, float x1, float x2, float y1, float y2)
        {
            ...
            if (radius > 0) {
                ...
                xmin = max(xmin, 0);
                ymin = max(ymin, 0);
                O2Log("Shadow: blur shadow - area : ((%.0f,%.0f),(%.0f,%.0f))",
                      xmin, ymin, xmax - xmin, ymax - ymin);
                // Stack blur - not really a gaussian one but much faster and
                // good enough Blur only the current clipped area - no need to
                // process pixels we won't see
                if (xmax > xmin && ymax > ymin) {
                    float r = shadowColor.r;
                    float g = shadowColor.g;
                    float b = shadowColor.b;
                    // This is bluring the alpha channel and fill pixels with
                    // (r,g,b) premultiplied by the blured alpha * the passed
                    // alpha parameter
                    partial_stack_blur_rgba32(*pixelFormatShadow, radius, xmin,
                                              xmax, ymin, ymax, r, g, b,
                                              shadowColor.a * alpha);
                } else {
                    O2Log("Skip blurring");
                }
            ...
            // Add the offset to the area to copy - the translated drawing only
            // take the rounded translation into account So add the fractionary
            // part
            Ras r;
            ...
            r.clip_box(ren_base->xmin(), ren_base->ymin(), ren_base->xmax(),
                       ren_base->ymax());

            // Rasterize the shadow to our main renderer
            agg::path_storage aggPath;

            O2Log("Shadow: copying to area : ((%.0f,%.0f),(%.0f,%.0f))", xmin,
                  ymin, xmax - xmin, ymax - ymin);
            // We'll use a path that cover the current used rect, and render
            // that path using the shadow image
            ...
            typedef agg::image_accessor_clone<pixfmt_shadow_type>
                img_accessor_type;
            ...
            transform.translate(
                -fract(shadowOffset.width),
                fract(shadowOffset.height)); // Translate using the fract part
                                             // of the shadow offset
            ...
            pixelFormat->comp_op(
                o2agg::comp_op_src_over); // Always use src_over when copying
                                          // the shadow
                                 ...
                                 T &type)
        {
            ...
            if (shadowColor.a > 0.) {
                ...
                ren_shadow->copy_bar(x - shadowBlurRadius, y - shadowBlurRadius,
                                     xmax + shadowBlurRadius,
                                     ymax + shadowBlurRadius,
                                     o2agg::rgba::no_color());
                ren_shadow->clip_box(ren_base->xmin(), ren_base->ymin(),
                                     ren_base->xmax(), ren_base->ymax());
                ...
                if (shadowBlurRadius == 0) {
                    // All colors are being transformed to the shadow one since
                    // we can skip the blur pass which is also applying the
                    // color
                    o2agg::rgba8 color =
                        o2agg::rgba(shadowColor.r, shadowColor.g, shadowColor.b,
                                    shadowColor.a * alpha);
                    span_color_converter color_converter(color, premultiply);
                    agg::span_converter<T, span_color_converter> converter(
                        type, color_converter);

                    // Render to the shadow location (rounded - we are working
                    // with pixels here) so we're sure we can properly blur the
                    // shadow We'll take into account the fractional part when
                    // copying the shadow to the final buffer
                    render_scanlines_aa_translate(rasterizer, sl, *ren_shadow,
                                                  span_allocator, converter,
                                                  truncf(shadowOffset.width),
                                                  -truncf(shadowOffset.height));
                } else {
                    // The bluring will actually apply the right shadow color -
                    // no need to do any transform here
                    render_scanlines_aa_translate(rasterizer, sl, *ren_shadow,
                                                  span_allocator, type,
                                                  truncf(shadowOffset.width),
                                                  -truncf(shadowOffset.height));
                ...
                // Get the used part of the rasterizer - we don't need to blur a
                // bigger area than that
            ...
            if (alpha >= 1) {
                ...
                agg::render_scanlines_aa(rasterizer, sl, *ren_base,
                                         span_allocator, type);
            } else {
                span_alpha_converter<o2agg::rgba8> alpha_converter(alpha,
                                                                   premultiply);
                agg::span_converter<T, span_alpha_converter<o2agg::rgba8>>
                    converter(type, alpha_converter);
                agg::render_scanlines_aa(rasterizer, sl, *ren_base,
                                         span_allocator, converter);
            }
        ...
        void render_scanlines_aa_solid(Ras &rasterizer, S &sl, T &type)
        {
            if (shadowColor.a > 0.) {
                ...
                // TODO : render to our scratch mask buffer, then blur should
                // just draw a shadow colored (* alpha) rect using that mask
                ///////////////////

                // Draw using our shadow rendererer using our shadow color (and
                // global alpha)
                T color =
                    o2agg::rgba(shadowColor.r, shadowColor.g, shadowColor.b,
                                shadowColor.a * type.opacity());
                color.premultiply();

                // Render to the shadow location (rounded - we are working with
                // pixels here) so we're sure we can properly blur the shadow
                // We'll take into account the fractional part when copying the
                // shadow to the final buffer
                render_scanlines_aa_solid_translate(
                    rasterizer, sl, *ren_shadow, color,
                    truncf(shadowOffset.width), -truncf(shadowOffset.height));
            ...
            if (premultiply)
                color.premultiply();
            if (color.opacity() >= 1. &&
                pixelFormat->comp_op() == o2agg::comp_op_src_over) {
                // We'll use copy instead of source over when rendering a opaque
                // path
    ...
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba<o2agg::rgba8, agg::order_rgba>>
        rgba_helper;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba<o2agg::rgba8, agg::order_bgra>>
        bgra_helper;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba<o2agg::rgba8, agg::order_argb>>
        argb_helper;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba<o2agg::rgba8, agg::order_abgr>>
        abgr_helper;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba_pre<o2agg::rgba8, agg::order_rgba>>
        rgba_helper_pre;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba_pre<o2agg::rgba8, agg::order_bgra>>
        bgra_helper_pre;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba_pre<o2agg::rgba8, agg::order_argb>>
        argb_helper_pre;
    typedef class context_renderer_helper<
        o2agg::comp_op_adaptor_rgba_pre<o2agg::rgba8, agg::order_abgr>>
        abgr_helper_pre;
  ...
  public:
    ...
    void init_pre(agg::rendering_buffer &renderingBuffer) {
        typedef o2agg::comp_op_adaptor_rgba_pre<o2agg::rgba8, Order>
            blender_type;
        premultiplied = true;
        helper = new context_renderer_helper<blender_type>(renderingBuffer);
        helper->setPremultiply(premultiplied);
    ...
    // Not sure how to better write that since we can't have virtual template
        ...
        if (premultiplied) {
            if (helper->isRGBA()) {
                ((rgba_helper_pre *) helper)
                    ->render_scanlines(rasterizer, sl, type);
            } else if (helper->isABGR()) {
                ((abgr_helper_pre *) helper)
                    ->render_scanlines(rasterizer, sl, type);
            } else if (helper->isBGRA()) {
                ((bgra_helper_pre *) helper)
                    ->render_scanlines(rasterizer, sl, type);
            } else if (helper->isBGRA()) {
                ((bgra_helper_pre *) helper)
                    ->render_scanlines(rasterizer, sl, type);
                             ...
                             T &type)
    {
        if (premultiplied) {
            if (helper->isRGBA()) {
                ((rgba_helper_pre *) helper)
                    ->render_scanlines_aa(rasterizer, sl, span_allocator, type);
            } else if (helper->isABGR()) {
                ((abgr_helper_pre *) helper)
                    ->render_scanlines_aa(rasterizer, sl, span_allocator, type);
            } else if (helper->isBGRA()) {
    ...
    void render_scanlines_aa_solid(Ras &rasterizer, S &sl, T &type)
    {
        if (premultiplied) {
            if (helper->isRGBA()) {
                ((rgba_helper_pre *) helper)
                    ->render_scanlines_aa_solid(rasterizer, sl, type);
            } else if (helper->isABGR()) {
                ((abgr_helper_pre *) helper)
                    ->render_scanlines_aa_solid(rasterizer, sl, type);
            } else if (helper->isBGRA()) {
...
template <> bool context_renderer::rgba_helper::isRGBA() {
    return true;
};
template <> bool context_renderer::bgra_helper::isBGRA() {
    return true;
};
template <> bool context_renderer::argb_helper::isARGB() {
    return true;
};
template <> bool context_renderer::abgr_helper::isABGR() {
    return true;
};
template <> bool context_renderer::rgba_helper_pre::isRGBA() {
    return true;
};
template <> bool context_renderer::bgra_helper_pre::isBGRA() {
    return true;
};
template <> bool context_renderer::argb_helper_pre::isARGB() {
    return true;
};
template <> bool context_renderer::abgr_helper_pre::isABGR() {
    return true;
};
                         ...
                         SpanGen &sg)
{
    if (self->useMask) {
        ...
        self->renderer->render_scanlines_aa(*self->rasterizer,
                                            *self->scanline_p8, sa, sg);
    }
}
                               ...
                               BOOL packed = NO)
{
    if (self->useMask) {
        ...
        if (packed) {
            self->renderer->render_scanlines_aa_solid(*self->rasterizer,
                                                      *self->scanline_p8, type);
        } else {
            self->renderer->render_scanlines_aa_solid(*self->rasterizer,
                                                      *self->scanline_u8, type);
        }
    }
}
                             ...
                             float x, float y, float width, float height)
{
    ...
    typedef agg::span_gradient<color_type, interpolator_type,
                               gradient_func_type, color_func_type>
        span_gradient_type;

    O2GState *gState = O2ContextCurrentGState(self);
    O2ClipState *clipState = O2GStateClipState(gState);
    O2AffineTransform deviceTransform = gState->_deviceSpaceTransform;
    agg::trans_affine deviceMatrix(deviceTransform.a, deviceTransform.b,
                                   deviceTransform.c, deviceTransform.d,
                                   deviceTransform.tx, deviceTransform.ty);

    O2Point deviceStart =
        O2PointApplyAffineTransform([shading startPoint], deviceTransform);
    O2Point deviceEnd =
        O2PointApplyAffineTransform([shading endPoint], deviceTransform);
    ...
    if ([shading isAxial]) {
        ...
        double scale = sqrt((deviceTransform.a * deviceTransform.a) +
                            (deviceTransform.c * deviceTransform.c));
    ...
    span_gradient_type span_gradient(span_interpolator, gradient_func,
                                     color_func, startValue, endValue);
                           ...
                           ImageInterpolationType interpolationType)
{
    ...
    if (resample == NO) {
        // Use a filter according to the wanted interpolation quality - no
        // resampling needed
        ...
        case kImageInterpolationRepeat: {
            typedef agg::image_accessor_wrap<pixfmt, agg::wrap_mode_repeat,
        ...
        // Use a filter according to the wanted interpolation quality - use
        // resampling
        switch (interpolationType) {
        default:
        case kImageInterpolationBilinear: {
            ...
            typedef o2agg::span_image_resample_rgba_affine<img_accessor_type>
                span_gen_type;
            span_gen_type sg(ia, interpolator, filter);
        ...
        case kImageInterpolationRepeat: {
            ...
            agg::image_filter_bilinear filter_kernel;
            agg::image_filter_lut filter(filter_kernel, true);
            // !!! don't use the o2agg version with a "repeat" accessor - it
            // seems some y wrapping is missing in the optimized resampling code
            // - so use the slower plain AGG version for now
...
#ifdef ANTIGRAIN_PRESENT
// If AntiGrain is not present it will just be a non-overriding subclass of the
// builtin context, so no problems
                                   ...
                                   const agg::trans_affine &deviceMatrix)
{
    ...
    if (gState->_dashLengthsCount > 1) {
        ...
        agg::conv_stroke<agg::conv_dash<agg::conv_curve<agg::path_storage>>>
            stroke(dash);
        ...
        for (int i = 0; i < gState->_dashLengthsCount; i += 2) {
            if (i == gState->_dashLengthsCount - 1) {
                ...
                dash.add_dash(gState->_dashLengths[i],
                              gState->_dashLengths[i + 1]);
            }
        }
        O2AGGContextSetStroke<typeof(stroke)>(self, stroke, deviceMatrix);
        O2AGGContextStrokePath<typeof(stroke)>(self, stroke, color,
                                               deviceMatrix);
    } else {
        // Just stroke
...
/* Transfer path from Onyx2D to AGG. Not a very expensive operation,
 * tessellation, stroking and rasterization are the expensive pieces
 */
                                ...
                                unsigned count)
{
    ...
    @synchronized([self class]) {
        ...
        NSString *fontName =
            [(NSString *) O2FontCopyFullName(font) autorelease];
        ...
        o2agg::rgba aggFillColor(fillComps[0], fillComps[1], fillComps[2],
                                 fillComps[3]);
        ...
        font_engine->hinting(
            false); // For some reason, it looks better without hinting...
        ...
        if (font_engine->create_font([fontName UTF8String],
                                     agg::glyph_ren_outline)) {
            agg::conv_transform<conv_curve_type, agg::trans_affine> trans(
                curves, transformMatrix);

            CGSize defaultAdvances[count];
            if (advances == NULL) {
                // Using a KTFont because we need to get the advancements the
                // same way the layout manager is getting them. Of course, it
                // would be better if the layout manager could give us the
                // advances it wants us to use...
                KTFont *kfont = [self _cachedKTFontWithFont: font
                                                       name: fontName
                                                       size: pointSize];
                [kfont getAdvancements: defaultAdvances
                             forGlyphs: glyphs
                                 count: count];
...
- initWithSurface: (O2Surface *) surface flipped: (BOOL) flipped {
    ...
    renderingBuffer = new agg::rendering_buffer(
        (unsigned char *) O2SurfaceGetPixelBytes(surface),
        O2SurfaceGetWidth(surface), O2SurfaceGetHeight(surface), bytesPerRow);

    rasterizer = new RasterizerType();
    scanline_u8 = new agg::scanline_u8;
    scanline_p8 = new agg::scanline_p8;

    // Use with the right order depending of the bitmap info of the surface -
    // we'll probably want to pass a pixel type here instead of an order to
    // support non 32 bits surfaces and pre/no pre
...
- (void) createMaskRenderer {
    // We use two gray masks to render our alpha mask - one for the current
    // mask, one to render a new clipping path using the current mask
    for (int i = 0; i < 2; ++i) {
        O2Surface *surface = [self surface];
        // For some still mysterious reason, if we don't allocate one additional
        // line, we sometimes get some crash when rendering the mask So we'll
        // just extend that buffer until we fix the problem at its root
        void *alphaBuffer = malloc(O2SurfaceGetWidth(surface) *
                                   (O2SurfaceGetHeight(surface) + 1));
        rBufAlphaMask[i] = new agg::rendering_buffer(
            (unsigned char *) alphaBuffer, O2SurfaceGetWidth(surface),
            O2SurfaceGetHeight(surface), O2SurfaceGetWidth(surface));
        alphaMask[i] = new MaskType(*rBufAlphaMask[i]);
        pixelFormatAlphaMask[i] = new pixfmt_alphaMaskType(*rBufAlphaMask[i]);
        baseRendererAlphaMask[i] =
            new BaseRendererWithAlphaMaskType(*pixelFormatAlphaMask[i]);

        solidScanlineRendererAlphaMask[i] =
            new agg::renderer_scanline_aa_solid<BaseRendererWithAlphaMaskType>(
                *baseRendererAlphaMask[i]);
...
- (void) setFillAlpha: (O2Float) alpha {
    // It's actually handled by the global alpha - that's the way it's supposed
    // to work in Cocoa
}
...
- (void) updateBlendMode {
    ...
    enum o2agg::comp_op_e blendModeMap[28] = {
        o2agg::comp_op_src_over,    o2agg::comp_op_multiply,
        o2agg::comp_op_screen,      o2agg::comp_op_overlay,
        o2agg::comp_op_darken,      o2agg::comp_op_lighten,
        o2agg::comp_op_color_dodge, o2agg::comp_op_color_burn,
        o2agg::comp_op_hard_light,  o2agg::comp_op_soft_light,
        o2agg::comp_op_difference,  o2agg::comp_op_exclusion,
        ...
        o2agg::comp_op_clear,       o2agg::comp_op_src,
        o2agg::comp_op_src_in,      o2agg::comp_op_src_out,
        o2agg::comp_op_src_atop,    o2agg::comp_op_dst_over,
        o2agg::comp_op_dst_in,      o2agg::comp_op_dst_out,
        o2agg::comp_op_dst_atop,    o2agg::comp_op_xor,
    ...
    if (shadowColor) {
        ...
        renderer->setShadowColor(o2agg::rgba(components[0], components[1],
...
- (void) updateMask {
    ...
    if ([savedClipPhases count]) {
        ...
        self->rasterizer->reset();
        self->rasterizer->clip_box(self->_vpx, self->_vpy,
        ...
        for (int i = 0; i < [savedClipPhases count]; ++i) {
            ...
            case O2ClipPhaseEOPath: {
                ...
                // We can ignore path that are rect - the default clipping takes
                // that into account
                NSRect rect;
                if (!O2PathIsRect(maskPath, &rect)) {
                    if (useMask == NO) {
                        ...
                        baseRendererAlphaMask[0]->clip_box(
                            self->_vpx - 1, self->_vpy,
                            self->_vpx + self->_vpwidth,
                            self->_vpy + self->_vpheight);

                        // We only need to fill the clipped rect area
                        baseRendererAlphaMask[0]->copy_bar(
                            self->_vpx, self->_vpy, self->_vpx + self->_vpwidth,
                            self->_vpy + self->_vpheight, agg::gray8(255, 255));
                    }
                    // Switch masks
                    currentMask = !currentMask;
                    // Clear the new mask
                    baseRendererAlphaMask[currentMask]->clip_box(
                        self->_vpx, self->_vpy, self->_vpx + self->_vpwidth,
                        self->_vpy + self->_vpheight);

                    // We only need to clear the clipped rect area
                    baseRendererAlphaMask[currentMask]->copy_bar(
                        self->_vpx, self->_vpy, self->_vpx + self->_vpwidth,
                        self->_vpy + self->_vpheight, agg::gray8(0));

                    buildAGGPathFromO2PathAndTransform(
                        &aggPath, maskPath, O2AffineTransformIdentity);
                    ...
                    renderer->render_scanlines(
                        *rasterizer, sl,
                        *solidScanlineRendererAlphaMask[currentMask]);
                }
                break;
            }
...
- (void) clipToState: (O2ClipState *) clipState {
    ...
    /*
 The builtin Onyx2D renderer only supports viewport clipping (one integer rect),
 so once the superclass has clipped the viewport is what we want to clip to
 also. The base AGG renderer also does viewport clipping, so we just set it.
 */
#ifdef O2AGG_GLYPH_SUPPORT
    O2GState *gState = O2ContextCurrentGState(self);
    if (gState->_shouldSmoothFonts == YES) {
        // We don't call the super method because we don't care about expensive
        // GDI clipping when we use no GDI for the drawing
    ...
    //  The real precise clipping mask for non-rect clipping will be built on
    //  demand
    O2ContextClipViewportToState(self, clipState);

    renderer->clipBox(self->_vpx, self->_vpy, self->_vpx + self->_vpwidth,
                      self->_vpy + self->_vpheight);

    // That will force a rebuild of the mask next time we need to draw something
    // - no need to build it now as it might change again before we need it for
    // drawing
    maskValid = NO;
}
...
- (void) drawPath: (O2PathDrawingMode) drawingMode {
    ...
    rasterizer->reset();
    rasterizer->clip_box(self->_vpx, self->_vpy, self->_vpx + self->_vpwidth,
                         self->_vpy + self->_vpheight);
    ...
    transferPath(self, (O2PathRef) _path,
                 O2AffineTransformInvert(gState->_userSpaceTransform));

    O2AffineTransform deviceTransform = gState->_deviceSpaceTransform;

    agg::trans_affine aggDeviceMatrix(deviceTransform.a, deviceTransform.b,
                                      deviceTransform.c, deviceTransform.d,
                                      deviceTransform.tx, deviceTransform.ty);
    ...
    if (doFill || doEOFill) {
        ...
        if (pattern) {
            ...
            // We don't make much use of the pattern phase, pattern color or
            // pattern bounds

            // Create a context with the size of our pattern tile
            O2Surface *surface = [self createSurfaceWithWidth: [pattern xstep]
                                                       height: [pattern ystep]];
            O2Context_AntiGrain *context =
                [[self->isa alloc] initWithSurface: surface flipped: YES];
            ...
            O2AffineTransform matrix = O2AffineTransformConcat(
                ([pattern matrix]),
                O2AffineTransformInvert(gState->_userSpaceTransform));

            agg::trans_affine globalTransform(matrix.a, matrix.b, matrix.c,
            ...
            agg::rendering_buffer imageBuffer(
                imgBytes, O2SurfaceGetWidth(surface),
                O2SurfaceGetHeight(surface), O2SurfaceGetBytesPerRow(surface));

            // Fill the current agg path with the repeated tile
            agg::conv_curve<agg::path_storage> curve(*(self->path));
            agg::conv_transform<agg::conv_curve<agg::path_storage>,
                                agg::trans_affine>
                trans(curve, aggDeviceMatrix);
            self->rasterizer->add_path(trans);
            renderer->premultiplied = YES;
            O2AGGContextDrawImage<o2agg::pixfmt_bgra32_pre>(
                self, imageBuffer, globalTransform, kImageInterpolationRepeat);
            ...
            o2agg::rgba aggFillColor(fillComps[0], fillComps[1], fillComps[2],
                                     fillComps[3]);
            if (doFill) {
                O2AGGContextFillPathWithRule(
                    self, aggFillColor, aggDeviceMatrix, agg::fill_non_zero);
            } else {
                O2AGGContextFillPathWithRule(
                    self, aggFillColor, aggDeviceMatrix, agg::fill_even_odd);
            }
            O2ColorRelease(fillColor);
        }
    }
    ...
    if (doStroke) {
        ...
        O2ColorRef strokeColor =
            O2ColorConvertToDeviceRGB(gState->_strokeColor);
        const float *strokeComps = O2ColorGetComponents(strokeColor);
        o2agg::rgba aggStrokeColor(strokeComps[0], strokeComps[1],
                                   strokeComps[2], strokeComps[3]);
...
- (void) drawImage: (O2Image *) image inRect: (O2Rect) rect {
    ...
    if (bpp == 32) { // We don't support other modes for now
        ...
        rasterizer->reset();
        rasterizer->clip_box(self->_vpx, self->_vpy,
                             self->_vpx + self->_vpwidth,
                             self->_vpy + self->_vpheight);
        ...
        p2.line_to(rect.origin.x + rect.size.width,
        ...
        agg::trans_affine transform(deviceTransform.a, deviceTransform.b,
                                    deviceTransform.c, deviceTransform.d,
                                    deviceTransform.tx, deviceTransform.ty);

        agg::conv_transform<agg::path_storage> trans(
            p2, transform); // Global Affine transformer
        rasterizer->add_path(trans);

        // Flipped scaled transfrom from the original image bounds to the
        // destination rect
        O2AffineTransform imageTransform = O2AffineTransformMakeTranslation(
            rect.origin.x, rect.origin.y + rect.size.height);
        imageTransform = O2AffineTransformScale(
            imageTransform, rect.size.width / float(width),
            -rect.size.height / float(height));

        agg::trans_affine globalTransform(imageTransform.a, imageTransform.b,
                                          imageTransform.c, imageTransform.d,
                                          imageTransform.tx, imageTransform.ty);
        globalTransform.multiply(transform);

        // We use bilinear scaling for images that need to be shrunk.  At 100%
        // scale size it preserves the image better. Remember we don't even
        // think about doing this for images that need to expand; those should
        // always use the bicubic filter; however,
        ...
        // Round the physical screen coordinate to an integer boundary so
        // filtering works better.  This seems to make images that are 1x1
        // rasterize
        //    exactly.  Changing the pixel coordinates on smaller scaled images
        //    introuduces unpleasant artifacts.
        if (isUnity) {
        ...
        case kO2BitmapByteOrder32Big:
            ...
            case kO2ImageAlphaPremultipliedLast:
                renderer->premultiplied = YES;
                O2AGGContextDrawImage<o2agg::pixfmt_rgba32_pre>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            case kO2ImageAlphaLast:
                renderer->premultiplied = NO;
                O2AGGContextDrawImage<o2agg::pixfmt_rgba32>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            case kO2ImageAlphaPremultipliedFirst:
                renderer->premultiplied = YES;
                O2AGGContextDrawImage<o2agg::pixfmt_argb32_pre>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            case kO2ImageAlphaFirst:
                renderer->premultiplied = NO;
                O2AGGContextDrawImage<o2agg::pixfmt_argb32>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            default:
                O2Log("UNKNOW ALPHA");
                // Fallback to the default implementation - that means path
                // clipping, shadow, layers... won't work well on this one
        ...
        case kO2BitmapByteOrder32Little:
            ...
            case kO2ImageAlphaPremultipliedLast:
                renderer->premultiplied = YES;
                O2AGGContextDrawImage<o2agg::pixfmt_abgr32_pre>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            case kO2ImageAlphaLast:
                renderer->premultiplied = NO;
                O2AGGContextDrawImage<o2agg::pixfmt_abgr32>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            case kO2ImageAlphaPremultipliedFirst:
                renderer->premultiplied = YES;
            ...
            case kO2ImageAlphaFirst:
                renderer->premultiplied = NO;
                O2AGGContextDrawImage<o2agg::pixfmt_bgra32>(
                    self, imageBuffer, globalTransform, interpolationType);
                break;
            default:
                // Fallback to the default implementation - that means clipping,
                // shadow, layers... won't work on this one
                O2Log("UNKNOW ALPHA");
                [super drawImage: image inRect: rect];
            }
            break;
        ...
        // Fallback to the default implementation - that means path clipping,
        // shadow, layers... won't work well on this one
        [super drawImage: image inRect: rect];
    }
}
...
- (void) drawShading: (O2Shading *) shading {
    ...
    if ([shading isAxial]) {
        O2AGGContextDrawShading<agg::gradient_x>(self, shading, _vpx, _vpy,
                                                 _vpwidth, _vpheight);
    } else {
        // Note: we only support circle gradient for now where startPoint =
        // endPoint
        O2AGGContextDrawShading<agg::gradient_circle>(self, shading, _vpx, _vpy,
                                                      _vpwidth, _vpheight);
    }
}

- (void) beginTransparencyLayerWithInfo: (NSDictionary *) unused {
    // NOTE: We could create a new context with only the current clipping size,
    // and offset all clipping/drawing from the clipping origin
    ...
    renderingBuffer->attach((unsigned char *) O2SurfaceGetPixelBytes(surface),
                            O2SurfaceGetWidth(surface),
                            O2SurfaceGetHeight(surface),
                            O2SurfaceGetBytesPerRow(surface));
...
- (void) endTransparencyLayer {
    ...
    // Reattach the renderer to the top surface (either to the new top layer if
    // any or the context one)
    O2LayerRef newTopLayer = [self->_layerStack lastObject];
    O2Surface *surface =
        newTopLayer ? O2LayerGetSurface(newTopLayer) : _surface;
    renderingBuffer->attach((unsigned char *) O2SurfaceGetPixelBytes(surface),
                            O2SurfaceGetWidth(surface),
                            O2SurfaceGetHeight(surface),
                            O2SurfaceGetBytesPerRow(surface));
    ...
    [self drawImage: O2LayerGetSurface(layer)
             inRect: O2RectMake(0, 0, size.width, size.height)];
              ...
              count: (unsigned) count
{
    ...
    // some issues when the context is scaled But rendering is faster
    O2GState *gState = O2ContextCurrentGState(self);
