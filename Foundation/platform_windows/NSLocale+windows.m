...
static BOOL CALLBACK enumLocalesProc(LPTSTR lpLocaleString) {
    ...
    // Gets the ISO lang and coutry for that locale and store the ISO<->LCID
    // association
    uint8_t langISO[9];
    GetLocaleInfo(lcid, LOCALE_SISO639LANGNAME, langISO,
                  sizeof(langISO) / sizeof(langISO[0]));
    ...
    (NSString *) string
{
    ...
    if (n) {
        ...
        uint16_t metrics[2];
        int size = GetLocaleInfoW(lcid, LOCALE_IMEASURE, metrics,
                                  sizeof(metrics) / sizeof(metrics[0]));
        if (size) {
            [desc setObject: [NSNumber numberWithBool: metrics[0] == '0']
                     forKey: NSLocaleUsesMetricSystem];
        }

        // Get the decimal separator
        uint16_t decimal[4];
        size = GetLocaleInfoW(lcid, LOCALE_SDECIMAL, decimal,
                              sizeof(decimal) / sizeof(decimal[0]));
        if (size) {
            [desc setObject: [NSString stringWithFormat: @"%S", decimal]
                     forKey: NSLocaleDecimalSeparator];
        }

        // Get the currency symbol
        uint16_t currency[19];
        size = GetLocaleInfoW(lcid, LOCALE_SCURRENCY, currency,
                              sizeof(currency) / sizeof(currency[0]));
        if (size) {
            [desc setObject: [NSString stringWithFormat: @"%S", currency]
...
+ (NSString *) _platformCurrentLocaleIdentifier {
    uint8_t langISO[9];
    GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SISO639LANGNAME, langISO,
                  sizeof(langISO) / sizeof(langISO[0]));
    uint8_t countryISO[9];
    GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SISO3166CTRYNAME, countryISO,
                  sizeof(countryISO) / sizeof(countryISO[0]));
