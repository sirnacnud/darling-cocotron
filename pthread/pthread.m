...
enum { condEvent_signal = 0, condEvent_broadcast = 1, condEventCount = 2 };
...
int pthread_cond_signal(pthread_cond_t *cond) {
    ...
    if (haveWaiters)
        SetEvent(impl->events[condEvent_signal]);
...
int pthread_cond_broadcast(pthread_cond_t *cond) {
    ...
    if (haveWaiters)
        SetEvent(impl->events[condEvent_broadcast]);
...
static inline void timespec_to_filetime(struct timespec *ts, FILETIME *ft) {
    LONGLONG ftime = ts->tv_sec * 10000000 + (ts->tv_nsec + 50) / 100 +
                     UNIX_EPOCH_START_AS_FILETIME;
    *((LONGLONG *) ft) = ftime;
}
                                ...
                                const struct timespec *abstime)
{
    ...
    _Bool lastWaiter = (result != WAIT_TIMEOUT) &&
                       (result == WAIT_OBJECT_0 + condEvent_broadcast) &&
                       (0 == newWaitersCount);

    if (lastWaiter)
        ResetEvent(impl->events[condEvent_broadcast]);
                   ...
                   void *(*start_routine)(void *), void *arg)
{
    ...
    // Win32 requires that a thread's start function uses the stdcall calling
    // convention, so we need a thunk to wrap the given start_routine. this
