...
- (void) testSuspending;
{
    ...
    STAssertTrue([queue isSuspended],
                 @"Queue should be suspended after setSuspended: YES");
...
- (void) testMaximumOperationCount;
{
    ...
    for (i = 1; i < 32; i++) {
        ...
        STAssertEquals(i, [queue maxConcurrentOperationCount],
                       @"Setting maxConcurrentOperationCount to %d should work",
                       i);
    }
}
...
- (void) testAddingWhileSuspended;
{
    ...
    STAssertFalse([operation isExecuting],
                  @"Operation should not be started while queue is suspended");
...
- (void) testOperationFinishedKVO;
{
    ...
    SleepWithRunloop(1);
    STAssertEquals((NSUInteger) 0, observationCount,
                   @"Should not have received isFinished notification yet");
    SleepWithRunloop(2);
    STAssertEquals((NSUInteger) 1, observationCount,
                   @"Should have received one isFinished notification");
}
