...
- (void) renderInContext: (CGContextRef) context {
    ...
    for (i = 0; i < numberOfModes; i++) {
        ...
        for (j = 0; j < numberOfBars; j++) {
            CGContextSetRGBFillColor(context, j / numberOfBars * 0.7,
                                     j / numberOfBars * 0.8,
                                     j / numberOfBars * 0.9, j / numberOfBars);
            CGContextFillRect(context, CGRectMake(0, i * height + j * barHeight,
                                                  1, barHeight));
        }

        CGContextSetBlendMode(context, modes[i]);
        for (j = 0; j < numberOfBars; j++) {
            CGContextSetRGBFillColor(context, j / numberOfBars * 0.9,
                                     j / numberOfBars * 0.7,
                                     j / numberOfBars * 0.8, j / numberOfBars);
            CGContextFillRect(context, CGRectMake(j * barWidth, i * height,
                                                  barWidth, height));
        }
    }
}
