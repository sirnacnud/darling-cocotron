...
- init {
    ...
    _context = CGBitmapContextCreate(_data, _pixelsWide, _pixelsHigh,
                                     _bitsPerComponent, _bytesPerRow,
...
- (void) drawClassic {
    ...
    CGContextAddArc(_context, CGRectGetMaxX(frame) - radius,
                    CGRectGetMaxY(frame), radius, M_PI_2, M_PI_2 * 3, YES);
    CGContextAddArc(_context, CGRectGetMinX(frame) + radius,
   ...
   CGContextClip(_context);
    ...
    if (_resamplingImage != NULL)
        CGContextDrawImage(_context,
                           CGRectMake(0, 0, CGImageGetWidth(_resamplingImage),
                                      CGImageGetHeight(_resamplingImage)),
                           _resamplingImage);
...
- (void) drawBlending {
    ...
    for (i = 0; i < limit; i++) {
        ...
        CGPathAddRect(path, NULL,
                      CGRectMake(i * width / limit, 0, width / limit, height));
    ...
    for (i = 0; i < limit; i++) {
        ...
        CGPathAddRect(path, NULL,
                      CGRectMake(0, 0, width, height - i * height / limit));
        CGContextAddPath(_context, path);
...
- (void) drawLayers {
    ...
    CGContextSetRGBFillColor(_context, 1, 1, 1, 1);
    CGContextFillRect(_context, CGRectMake(0, 0, 400, 400));
    CGContextSetShadowWithColor(_context, CGSizeMake(30, 30), _shadowBlur,
                                _shadowColor);
